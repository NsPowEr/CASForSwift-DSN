...
std::string LaTeXFormatter::format(ExprPtr expr) {
    ...
    return visit_expr(expr, [this](const auto& node) -> std::string {
        ...
        if constexpr (std::is_same_v<NodeT, IntegerLit>) {
            ...
            if (node.func_id == BuiltinOp::Sqrt) return "\\sqrt{" + format(node.args[0]) + "}";
            if (node.func_id == BuiltinOp::Sin || node.func_id == BuiltinOp::Cos || node.func_id == BuiltinOp::Tan || node.func_id == BuiltinOp::Ln || node.func_id == BuiltinOp::Exp) {
                return "\\" + node.name + "{" + format(node.args[0]) + "}";
            }
            if (node.func_id == BuiltinOp::Gamma || node.name == "Gamma") {
                return "\\Gamma(" + format(node.args[0]) + ")";
            }
