...
CASErrorRef cas_evaluate_numeric(CASContextRef ctx, CASExprRef expr, CASExprRef* out_expr) {
    if (!ctx || !expr || !out_expr) {
        return new CASError{"Invalid arguments to cas_evaluate_numeric"};
    }

    try {
        auto res = cas::numeric::eval(expr->ptr);
        if (res.is_error()) {
            return new CASError{res.error().message};
        }

        *out_expr = new CASExpr{ctx->impl.arena().make<cas::DecimalLit>(res.value())};
        return nullptr;
    } catch (const std::exception& e) {
        return new CASError{std::string("C++ Exception in cas_evaluate_numeric: ") + e.what()};
    } catch (...) {
        return new CASError{"Unknown C++ Exception in cas_evaluate_numeric"};
    }
}
