...
Result<bool> mathematically_equal(ExprPtr lhs, ExprPtr rhs, CASContext& context) {
    ...
    // Rational equality: check num_L * den_R - num_R * den_L == 0
    ...
    if (lhs_parts.is_ok() && rhs_parts.is_ok()) {
        ...
        if (cross_l.is_ok() && cross_r.is_ok()) {
            auto diff_expr = context.arena().make<Binary>(BinaryOp::Sub, cross_l.value(), cross_r.value());
            auto diff = algebra::expand(diff_expr, context);
            if (diff.is_ok() && algebra::is_zero_expr(diff.value())) {
                finalize();
                return ok(true);
            }
        }
