// A53 (anti-monolito) — implementazione delle guardie del definito, estratte da
// integrate.cpp. Vedi integrate_definite_guards.hpp per il contratto.
...
[[nodiscard]] ExprPtr normalize_definite_integrand_impl(ExprPtr expr, symbolic::CASContext& ctx) {
    auto together_expr = algebra::together(expr, ctx);
...
Result<void> reject_poles_in_closed_interval(
    ExprPtr integrand, const Symbol& var, ExprPtr lower, ExprPtr upper,
    symbolic::CASContext& ctx) {
    return reject_rational_poles_in_closed_interval(integrand, var, lower, upper, ctx);
}

ExprPtr normalize_definite_integrand(ExprPtr expr, symbolic::CASContext& ctx) {
    return normalize_definite_integrand_impl(expr, ctx);
}
