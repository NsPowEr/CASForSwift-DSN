...
- **Vero prerequisito (REGOLA ZERO: costruisci prima il prerequisito)**: robustezza del simplifier/`together` sui radicali — cancellazione GCD su atomo radicale (`(√(x²+1)+x)` ≡ fattore di `x+√(x²+1)`) e gestione di `Pow(conj, k)`/`Pow(−1, −1)` senza corruzione di esponenti. Solo costruito quello, la razionalizzazione coniugata (questo entry) diventa un patch limitato. Stima rivista: research-grade (C4), non bounded.
- **STATO**: APERTO (versione di passaggio: anti-hang attivo, forma chiusa NON prodotta; tentativo razionalizzazione bloccato dalla fragilità simplifier-su-radicali).
