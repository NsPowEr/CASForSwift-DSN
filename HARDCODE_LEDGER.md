...
### HC-A26-RDEBOUND-CANCEL-GAP — degree bound RdeBoundDegreePrim (Log, df≤0): branch di cancellazione §6.3.3 non gestito — 📋 APERTO 2026-07-20
...
- **Categoria CLAUDE.md**: 1 (bound computazionale — ramo del bound matematico non gestito). **Sound, non silent-wrong**: l'incompletezza risale come `Unimplemented`, mai come integrale sbagliato.
...
- **Sintomo**: la formula completa `RdeBoundDegreePrim` (Bronstein §6.3.3) prevede, per `d_b = d_a` e `d_b = d_a − 1`, un incremento ulteriore di n quando il coefficiente di testa `α = −lc(b)/lc(a)` è una derivata logaritmica in `k` (`α = Dz/z`) e la conseguente `β` è `s·Dt + Dw` con `s ∈ Z`. Questo branch qui NON è implementato: il bound si ferma a `dg_max+1`. Se in un caso reale la cancellazione di testa si verifica, N resta sottostimato e la ricorsione per-grado non esplora il grado della soluzione → la famiglia parametrica torna incompleta.
- **Perché è accettabile ora**: (1) è sound — ogni soluzione trovata è verificata per back-substitution, quindi una soluzione mancante fa solo tornare una famiglia più piccola, che a monte (`limited_integrate_field`, `integrate()`) risale come `Unimplemented`, MAI come integrale sbagliato (contratto REGOLA ZERO rispettato); (2) è un MIGLIORAMENTO rispetto al pre-esistente `dg_max` (già più basso, gap più ampio) — questo diff riduce il gap, non lo introduce; (3) il caso primitivo di base target di HC-A26 non lo attraversa (verificato: `q = x·log x` catturato da `dg_max+1`).
- **Fix corretto**: implementare il branch §6.3.3 (riconoscimento derivata logaritmica in `k` via §5.12 + limited-integration in `k` via LimitedIntegrateReduce §7.2 applicato un livello sotto nella torre), riusando il pattern "degree bound con caso cancellation" già presente in `risch_parametric_rational.cpp` (HC-A26 fase 3). In alternativa, guard esplicito che emetta `Unimplemented` diagnostico quando la condizione di cancellazione di testa è rilevata, invece di procedere con N potenzialmente insufficiente.
- **Blocking dependency**: nessuna bloccante (riusa primitive già in codebase); la versione completa dipende da §5.12/§7.2 ricorsivi sulla torre.
