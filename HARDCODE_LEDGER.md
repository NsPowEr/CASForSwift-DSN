...
- **🔧 RESIDUO APERTO**: il caso **f≠0 razionale** (integrating factor `exp(−∫f)` generalmente non razionale) richiede la chain Bronstein completa NormalDenominator+BoundDegreeBase+ParamPolyRischDE — multi-giorno, verbatim. Per ora `solve_param_limited_integration_rational_q` gestisce solo f=0; residui **algebrici** (non razionali) → Unimplemented pulito. Wiring in `integrate()` resta subordinato al completamento f≠0.
- **STATO**: 🔨 PARZIALE (f=0 risolto+wired; f≠0 razionale + residui algebrici pending). **Refs**: A26, A1, HC-F8-PENDING-17.
