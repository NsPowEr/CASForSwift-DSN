...
- **Residuo (famiglia estesa, non bloccante)**: `∫log(x+√(x²+a))` con `a≠1` torna ancora NO_STRATEGY (non-wrong, solo non risolto, ~25s) perché `together` produce `c·x·(c·√(x²+a))⁻¹` (es. `16·x·(16·√)⁻¹` per `a=4`, dove `c=(S·conj)²`) e il simplifier NON estrae il coefficiente numerico da `Pow(c·rest, n<0)` → `c^n·Pow(rest,n)`: è il gap **T-054** (`simplify_arithmetic_power.cpp:346-354`), esplicitamente differito perché farlo nel power-path universale rompe Gruntz (regressione mrv documentata). Il caso `acosh` (`x²−1`) fallisce per ragione distinta (`R=S·conj=1`, niente coefficiente; probabile gestione segno/dominio di `√(x²−1)`). Entrambi sbloccati quando T-054 normalizza i coefficienti numerici al layer Sum/like-term.
