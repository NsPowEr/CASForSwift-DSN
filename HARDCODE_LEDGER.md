...
- **⚠️ BUG MOTORE SCOPERTO (separato)**: `simplify`/`together`/`diff` corrompono frazioni `(coeff_razionale · monomio)/potenza` — es. `(−x/2)/x² → −1/x` (perde il fattore 1/2). Perciò la verifica f≠0 usa il residuo Q esatto (non il simplifier) e l'output è integer-cleared. **Affligge potenzialmente altri risultati** del motore → investigare/fixare separatamente in `simplify_core`/`together` (nuovo ticket).
