...
> Aggiornato: 2026-05-06 (audit critico completo — regressioni marcate, stati corretti)  
...
8. **Fattorizzazione Galois**: Fattorizzazione su estensioni algebriche assente (Score 0). → CAS-L3-06, CAS-L3-18
9. **GCD Multivariato**: Non implementato pienamente (Score 2). → CAS-L1-08
10. **Semplificazione e Algebra RootOf**: `RootOf` è inerte, non semplificabile e non valutabile. → CAS-L1-05
...
17. **Limiti (MRV)**: Algoritmo a 3 livelli hardcoded, fallisce su torri esponenziali. → CAS-L1-01
...
| CAS-L2-08 | Complex | Polar/Log Completo | L2 | Aperta | — | Medio | arg/abs/ln complessi |
...
| CAS-L2-11 | Calculus | Integrali Impropri e Valore Principale | L2 | Aperta | L1-06, L2-05 | Molto Alto | Convergenza + Cauchy PV + residui |
...
| CAS-L2-15 | Polynomial | Risolutore ciclotomica grado arbitrario | L2 | Aperta | L0-09 | Basso | Fattorizzazione ciclotomica minimale; polynomial_cyclotomic.cpp:85-88 cerca solo fino a n=100 → x^101-1=0 non fattorizzato |
...
| CAS-L2-22 | Calculus | Integrali Definiti via Residui | L2 | Aperta | L2-11, L2-05 | Alto | Contour/residue per casi classici |
...
| CAS-L2-27 | Calculus | Serie Taylor Generatore Sistematico | L2 | Aperta | L2-05 | Alto | limit_series.cpp tabella hardcoded per sin/cos/exp/ln/arctan; tan(x), W(x), funzioni composte arbitrarie non gestite; serve generatore via derivate successive |
...
| CAS-L3-04 | SpecialFn | Funzioni Speciali Core | L3 | Aperta | L1-02 | Molto Alto | Gamma/erf/Bessel/Legendre base |
| CAS-L3-05 | Calculus | Zeilberger | L3 | Aperta | L1-01 | Medio | Summazione ipergeometrica |
| CAS-L3-06 | Algebra | Fattorizzazione su Estensioni | L3 | Aperta | L1-04, L1-05, L3-14 | Molto Alto | Fattori in Q(a) e splitting; dispatcher in factorization_polynomials.cpp:649 rifiuta estensioni non-sqrt con `solo sqrt(n) per ora` |
...
*   **CAS-L0-05 (Polynomial)**: Rimuovere seed fisso `42` in Cantor-Zassenhaus. Sostituire con seed derivato dall'hash del polinomio in input o parametro esplicito. **(PARZIALE)**
    *   **Problema residuo**: `select_factorization_prime()` in `factorization_integers.cpp` → fallback deterministico a `BigInt(13)` se tutti i candidati dividono il leading coefficient. Non usa hash del polinomio.
    *   **Piano di risoluzione**:
        1. Calcolare hash FNV-1a del vettore di coefficienti del polinomio.
        2. Usare lo hash come seed per selezione pseudorandom tra un pool allargato di primi (es. `{13,17,...,97}`).
        3. Se il primo selezionato divide lc, iterare circolarmente sul pool (non tornare a 13).
        4. Aggiungere test: `factorize(p)` due volte su polinomio con lc divisibile da 13 → deve scegliere primo diverso dal fallback deterministico.
...
*   **CAS-L1-01 (Gruntz)**: MRV ricorsivo esteso per x→±∞, torri esponenziali e comparabili dello stesso ordine con coefficiente leader esatto. Resta parziale: se il leading term cancella e serve il termine successivo della serie, il motore deve restare `Unimplemented`.
...
*   **CAS-L1-05 (RootOf)**: Trasformare `RootOf` da terminatore inerte a operatore algebrico semplificabile e valutabile numericamente. **(PARZIALE)**
    *   **Problema residuo**: `RootOf` usato solo come segnaposto in output. Non supporta: `RootOf(x^2-2)^2 → 2`, valutazione numerica `eval_to_float()`, divisione per `RootOf`.
    *   **Piano di risoluzione**:
...
*   **CAS-L2-05 (Serie Laurent)**: Espansioni in serie con esponenti negativi (poli). Base indispensabile per calcolo residui, integrali impropri (L2-11), Padé (L2-12).
...
*   **CAS-L2-22 (Residui Definiti)**: Usare residue theorem per integrali definiti classici, complementare a impropri/PV.
...
*   **CAS-L3-05 (Zeilberger)**: Completare implementazione stub Zeilberger per summazione ipergeometrica. Algoritmo Petkovšek/WZ completo.
*   **CAS-L3-06 (Fattorizzazione Estensioni)**: Fattorizzazione su estensioni algebriche (`Q(a)`) per chiudere gap con `RootOf`.
