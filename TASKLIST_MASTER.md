...
- **🔧 RESIDUO (famiglie DIVERSE, non più il trig-rational)**: restano fail (a) integrate **algebrici/inverse-hyp** — `∫1/sqrt(x²+1)`, `∫sqrt(x²−1)`, `∫x²/(x²−1)`, `∫1/sqrt(a²−x²)` (non-trig, gap separato); (b) `diff(x/sin x)` = **polinomio×trig misto** (`x` come Symbol nudo × trig → il convertitore Laurent fa bail sound, non è un razionale trig puro); (c) `∫1/(2+cos x)` e simili con antiderivata **`arctan`**: `D(cas)` va in **timeout** (`Symbolic operation timed out`) sul `arctan(tan(x/2)/√3)` annidato — sub-gap **perf di `diff`**, upstream del convertitore. Tutti C4·R3, sessioni a sé.
