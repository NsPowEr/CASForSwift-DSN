...
### A6 · Stauduhar Galois group deg ≥ 6 — 🔨 IN CORSO 2026-07-11 (inc.1-5 + Brick 1/4 deg≥8: perm-engine + lattice + driver esatto deg 6-7 + BSGS scalabile) — `[E4·C4·S3·R1]`
...
- **🔧 PROSSIMI INCREMENTI (Brick 2-4, sessioni successive)**: (2) discesa top-down sui **massimali transitivi** calcolati on-demand via BSGS (block systems + primitivi), niente pre-enumerazione — la transitività è up-closed; (3) risolventi relative G/H esatte (generalizzano `galois_setpoly.cpp`) + fattorizzazione su Q, con i raffinamenti che separano le coppie Galois-equivalenti a n=8; (4) naming strutturale classi n=8..10 (da invarianti del gruppo generato) + wiring `galois.cpp` + corpus oracoli ≥30.
- **Nota architetturale**: approccio **interamente esatto e generato** (niente float, niente trascrizioni) anche per n ≥ 8 — Fieker-Klüners esatto, NON lo Stauduhar-con-radici-BigFloat della voce storica/spec. `HC-F8-PENDING-09` resta APERTA fino al Brick 4.
...
### A31 · Propagazione condizioni di dominio (`x≠0`) attraverso simplify — 🆕 APERTO 2026-07-10 — `[E4·C4·S3·R2]`
...
- **Cosa serve davvero**: il motore deve **portare** le condizioni. Opzioni da valutare in spec: (a) risultato `Piecewise`/`ConditionalExpr` con dominio esplicito; (b) accumulo di side-conditions in `CASContext` durante `simplify`, esposte nel `Result`; (c) semantica "generic point" dichiarata (come Maxima/SymPy: `x/x → 1` sul dominio di definizione) e allora i due cambi vanno **respinti per design**, non rimandati.
- **Nota**: incoerenza pre-esistente da risolvere insieme — `Div(x,x)` e `x·x⁻¹` in forma `Product` seguono oggi regole diverse.
- **Patch di riferimento**: `foreign_wip_backup.patch` (job tmp) · **Refs**: F8.0-6.2, Branch_Cut_Propagation.md
