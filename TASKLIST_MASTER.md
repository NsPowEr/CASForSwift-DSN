...
### A26 · Wire-in solver parametrico Risch DE a livello tower (prerequisito A1 df>0) — 🔨 IN CORSO (f=0 + f≠0 razionale chiusi 2026-06-30; residuo WeakNormalizer) — `[E4·C4·S3·R2]`
...
- **✅ FASE 3 — caso f≠0 razionale RISOLTO 2026-06-30** (commit `7725c25`): nuovo `solve_param_risch_de_rational_q` = **ParamRischDE generale su Q(x)** (Bronstein §6.1/§7.1). Ansatz `y=P/D`, `D=lcm(den f,den g_i)`; identità `D·P'+(Fn−D')·P−Σc_i(Gn_i·D)=0` → sistema omogeneo → null space. **Degree bound cancellation** (`deg_H==deg_D−1` → grado omogeneo `−lc(H)/lc(D)`; trova `y=x²` di `y'−2y/x=0`). **Verifica engine-independent** (residuo Q esatto, non simplifier) + output integer-cleared. Wired (ext_idx==0, fallback f≠0). +4 test. 85 verdi, 0 regressioni.
- **⚠️ BUG MOTORE scoperto** (ledger `HC-SIMPLIFY-RATCOEFF-OVER-POWER`, priorità alta): `simplify`/`together`/`diff` perdono il coeff razionale in `(c·monomio)/potenza` — `(−x/2)/x²→−1/x`. **Silent-wrong**, potenzialmente diffuso → ticket separato. A26 lo aggira (residuo Q + integer-clearing).
- **🔧 RESIDUO APERTO A26 (ledger HC-A26)**: **WeakNormalizer** (Bronstein 6.1.1) — poli semplici di f con residuo intero-negativo danno `den(y)` oltre `lcm` → soluzioni silenziosamente assenti (incompletezza, MAI sbagliato; stessa limitazione del solver non-parametrico). Residui **algebrici** → Unimplemented pulito. Wiring in `integrate()` reale subordinato a questo + df>0 (A1).
