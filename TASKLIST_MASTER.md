...
- **🔧 RESIDUO (real-domain `abs`, NON forzare — vd. B.1)**: restano fail (a) integrate **inverse-hyp/algebrici a dominio reale** — `∫1/sqrt(x²+1)` (CAS `ln|x+√(x²+1)|` vs Maxima `asinh x`: il bridge `asinh→ln` esiste già in `hyperbolic_normalize`, blocca solo l'**`abs`** — `x+√(x²+1)>0` vale solo su ℝ, provarlo richiede ipotesi reale + lemma `√(x²+1)>|x|`), `∫sqrt(x²−1)` (CAS emette `unknown(x)` per la parte log → **bug integrazione separato**, non equivalenza), `∫1/sqrt(a²−x²)` (`arcsin(x/a)` vs `arcsin(x/|a|)`, `abs` su parametro, vale solo `a>0`); (b) `diff(x/sin x)` = **polinomio×trig misto** (`x` come Symbol nudo × trig → il convertitore Laurent fa bail sound, non è un razionale trig puro). I casi (a) `abs` sono la stessa tensione real-vs-complex di B.1: fix corretto = **default real-domain configurabile** (decisione cross-cutting), non hack per-caso.
