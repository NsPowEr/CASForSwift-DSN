...
### A26 · Wire-in solver parametrico Risch DE a livello tower (prerequisito A1 df>0) — 🔨 IN CORSO (fase 2 f=0 chiusa 2026-06-30; residuo f≠0) — `[E4·C4·S3·R2]`
...
- **🔧 RESIDUO APERTO (ledger HC-A26-PRIMITIVE-PARAMQ-RATIONAL)**: caso **f≠0 razionale** (integrating factor `exp(−∫f)` non razionale) richiede chain Bronstein completa NormalDenominator+BoundDegreeBase+ParamPolyRischDE; residui **algebrici** (non razionali) → Unimplemented pulito. Wiring in `integrate()` reale subordinato a questo + completamento df>0 (A1).
