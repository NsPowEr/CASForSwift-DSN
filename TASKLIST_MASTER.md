...
- **🔧 RESIDUO (famiglie DIVERSE, non più il trig-rational)**: restano fail (a) integrate **algebrici/inverse-hyp** — `∫1/sqrt(x²+1)` (CAS `ln|x+√(x²+1)|` vs Maxima `asinh x`), `∫sqrt(x²−1)` (CAS emette `unknown(x)` per la parte log), `∫x²/(x²−1)` (CAS `x−I·arctan(−I·x)` vs Maxima `x−½log(x+1)+½log(x−1)` — log↔arctan-complesso), `∫1/sqrt(a²−x²)` (`arcsin(x/a)` vs `arcsin(x/|a|)`, gap `abs` su parametro); (b) `diff(x/sin x)` = **polinomio×trig misto** (`x` come Symbol nudo × trig → il convertitore Laurent fa bail sound, non è un razionale trig puro).
