...
### A27 · Log-derivative recognition per torri log ANNIDATE (nested) — 🆕 APERTO 2026-07-01 — `[E2·C3·S2·R1]`
- **Gap osservato** (probe batch 2026-07-01): `∫ 1/(x·log(x)·log(log(x))) dx = log(log(log(x)))` è **elementare** ma `integrate()` ritorna `INTEGRATE_NO_STRATEGY` Unimplemented. È `D(log(g))` con `g = log(log x)` (generatore top t_2), `c=1` → dovrebbe cadere nello step 2b (log-derivative recognition, `integrate_risch.cpp:97-164`) che però itera solo generatori diretti e non riconosce la catena annidata a 3 livelli.
- **Fix corretto**: estendere la ricerca candidati 2b a generatori annidati (comporre `g = FuncCall(Ln, ext.argument)` quando `ext.argument` è a sua volta un generatore log) + verifica round-trip `D(c·log(g))−integrand=0` (già presente). NON è compito del solver parametrico. Sound-by-verify.
- **Refs**: A1/A26 (stessa area Risch, indipendente) · **Non è un silent-wrong** (bail pulito).
