...
### A26 · Wire-in solver parametrico Risch DE a livello tower (prerequisito A1 df>0) — 🔨 IN CORSO 2026-06-29 (validato+esposto; primitivo residuo) — `[E4·C4·S3·R2]`
...
- **🔧 RESIDUO APERTO (gap primitivo, ledger HC-A26-PRIMITIVE-PARAMQ-RATIONAL)**: il ramo log (primitivo) genera dalla correzione `i·y·θ'` (θ'=1/x) una forzante **razionale** che il base case `solve_risch_de_parametric_q` (solo `Q[x]`) non gestisce → Unimplemented diagnostico (no silent-wrong, no hang; REGOLA ZERO ok). Completamento = ParamRischDE su `Q(x)` razionale (weak-normalizer Bronstein 6.1.1 + denominator bound). Solo a primitivo completo la funzione è wire-in-abile in `integrate()` senza rischio.
