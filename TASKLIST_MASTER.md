...
- **🔧 RESIDUO (sub-gap separato, NON ancora chiuso)**: i golden entry `∫1/sin x`, `∫1/(2+cos x)`, `diff(sin/cos)`… **restano fail**. Causa: le antiderivate CAS derivano da forme con **`abs`** (`ln|tan(x/2)|`) e da **Weierstrass half-angle** la cui derivata (`D(cas)` confrontata in `antiderivative_equivalent`) porta artefatti abs/half-angle che `simplify`+`split_num_den` non riducono a un razionale trig pulito → `trig_exponential_zero_diff` fa bail. Fix futuro: (a) ridurre il derivato di `ln|tan(x/2)|` (drop abs in derivata: `d/dx ln|u|=u'/u`) a forma trig razionale pulita, oppure (b) estendere il convertitore Laurent a gestire `split_num_den` su forme half-angle annidate. Resta C4·R3.
