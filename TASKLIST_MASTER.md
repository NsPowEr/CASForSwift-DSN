...
### A6 · Stauduhar Galois group deg ≥ 6 — 🔨 IN CORSO 2026-07-10 (inc.1-5 fatti: perm-engine + lattice ottimizzato + driver esatto deg 6-7) — `[E4·C4·S3·R1]`
...
- **🔧 PROSSIMO INCREMENTO**: (6) deg 8..10 = discesa Stauduhar sui massimali + radici certificate interval-Newton (Aberth esiste ma non certificato). Il lattice denso non scala oltre n=7 (memoria Θ(n!)/sottogruppo, tabella Cayley (n!)²): serve l'enumerazione top-down per massimali (transitività è up-closed). Naming strutturale classi non-famose oltre le etichettate.
- **Nota architetturale**: la via "tabelle Hulpke n=6..10 + risolventi numeriche BigFloat" della voce storica è sostituita per n≤7 da un approccio **interamente esatto e generato** (niente float, niente trascrizioni). BigFloat/intervalli entrano solo dagli incrementi Stauduhar n≥8.
