...
struct FuncCall : ExprNode {
    ...
    FuncCall(std::string function_name, std::vector<ExprPtr> function_args)
        : ExprNode(KIND), name(std::move(function_name)), args(std::move(function_args)) {}
...
public:
    ...
    [[nodiscard]] ExprPtr make(Args&&... args) {
        ...
        // Path veloci per costanti calde
        if constexpr (std::is_same_v<T, Constant>) {
            Constant candidate(std::forward<Args>(args)...);
            ...
            ExprPtr created = make_uncached<T>(candidate.value);
        ...
        if constexpr (std::is_same_v<T, IntegerLit>) {
            IntegerLit candidate(std::forward<Args>(args)...);
            ...
            if (static_cast<const IntegerLit*>(created.get())->value.is_zero()) interned_zero_ = created;
            else if (static_cast<const IntegerLit*>(created.get())->value == BigInt(1)) interned_one_ = created;
            else if (static_cast<const IntegerLit*>(created.get())->value == BigInt(-1)) interned_negative_one_ = created;
        ...
        // Se non trovato, allochiamo realmente
        ExprPtr created = make_uncached<T>(std::move(candidate));
        interning_table_.insert(created);
        return created;
    }
