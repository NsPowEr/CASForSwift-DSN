...
[[nodiscard]] bool depends_on(ExprPtr expr, const Symbol& var) {
    ...
    if (const auto* integral = expr_cast<Integral>(expr)) {
        ...
        if (depends_on(integral->integrand, var)) {
            return true;
        }
        return (integral->lower.has_value() && depends_on(*integral->lower, var)) ||
            (integral->upper.has_value() && depends_on(*integral->upper, var));
    }
...
private:
    ...
    [[nodiscard]] Result<ExprPtr> differentiate_function(const FuncCall& call, const Symbol& var) {
        if (call.args.size() != 1U) {
            return fail<ExprPtr>(make_error(CASErrorKind::Unimplemented, "Only unary function differentiation is implemented"));
        }
        ...
        auto transc_res = differentiate_transcendental(name, argument, var, context_);
        if (transc_res.is_ok()) {
            return transc_res;
        }
