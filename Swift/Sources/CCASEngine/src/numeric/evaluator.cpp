    ...
    return visit_expr(
        ...
        [this](const auto& node) -> Result<double> {
            ...
            if constexpr (std::is_same_v<NodeT, IntegerLit>) {
                ...
                if (node.name == "sin") return ok(std::sin(args[0]));
                if (node.name == "cos") return ok(std::cos(args[0]));
                if (node.name == "tan") {
                    double val = std::tan(args[0]);
                    if (std::isinf(val)) return fail<double>(make_error(CASErrorKind::Undefined, "Tan: Singular point"));
                    return ok(val);
                }
                if (node.name == "exp") return ok(std::exp(args[0]));
                ...
                if (node.name == "abs") return ok(std::abs(args[0]));
