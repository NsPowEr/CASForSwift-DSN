...
[[nodiscard]] bool range_is_exact_zero(ExprPtr lower, ExprPtr upper) {
    auto exact_lower = exact_scalar_from_expr(lower);
    auto exact_upper = exact_scalar_from_expr(upper);
...
[[nodiscard]] bool exact_range_excludes_zero(ExprPtr lower, ExprPtr upper) {
    ...
    return compare_exact_scalars(*exact_lower, Rational(BigInt(0))) > 0 || compare_exact_scalars(*exact_upper, Rational(BigInt(0))) < 0;
}
